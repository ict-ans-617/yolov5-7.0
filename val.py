...
def run(
    ...
    if training:  # called by train.py
        ...
        # print(model)
        # checkpoint = torch.load(weights, map_location=torch.device('cpu'))  # Load onto CPU
    ...
    # Dataloader
    if not training:
        if pt and not single_cls and hasattr(model.model, 'nc'):  # check --weights are trained on --data
            ncm = model.model.nc
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'pruned_yolov5s_voc_FPGM_0.2.pt', help='model path(s)')
